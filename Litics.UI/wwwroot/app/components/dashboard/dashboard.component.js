function DashboardCtrl($scope, $log, $timeout) {
    let vm = this;
    ...
    $scope.dashboard = {
        widgets: [{
            ...
            directive: '<charts widget="widget" config="config" events="events"></charts>'
        },  {
            col: 4,
            row: 2,
            sizeY: 1,
    ...
    $scope.addWidget = function () {
        console.log($scope.customItemMap);
        $scope.customItems.push({ size: { x: 4, y: 1 }, position: [0, 0], template: '<line-plot graph-plots=graphPlots></line-plot>' });
    };
}
