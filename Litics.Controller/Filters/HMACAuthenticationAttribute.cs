...
namespace Litics.Controller.Filters
{
    public class HMACAuthenticationAttribute : Attribute, IAuthenticationFilter
    {
        private static ConcurrentDictionary<string, string> allowedApps = new ConcurrentDictionary<string, string>();
        ...
        public HMACAuthenticationAttribute()
        {
            if (allowedApps.Count == 0)
            {
                ...
                {
                    var allowedAppsDb = context.Account.Where(where => where.AppId != null && where.ApiKey != null).Select(acc => new { acc.AppId, acc.ApiKey }).ToDictionary(x => x.AppId, x => x.ApiKey);
                    allowedApps = new ConcurrentDictionary<string, string>(allowedAppsDb);
                }
            }
        }
